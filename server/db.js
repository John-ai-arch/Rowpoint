...
/* -------------------- AI stroke analysis platform (moat #2) --------------------
   An extensible video-analysis platform. A "stroke analysis" stores the timeline
   marks (catch/finish timestamps — today entered with assistance, tomorrow
   auto-generated by a pose-estimation module behind the same interface), the
   derived metrics + coaching observations produced by the modular pipeline
   (server/stroke/pipeline.js), and coach/athlete annotations. Video bytes are
   referenced by video_ref (client-side or future object storage) so the schema
   never bloats with blobs. */
db.exec(`
...
CREATE INDEX IF NOT EXISTS idx_stroke_user ON stroke_analyses(user_id, created_at);

CREATE TABLE IF NOT EXISTS stroke_annotations (
  id TEXT PRIMARY KEY,
