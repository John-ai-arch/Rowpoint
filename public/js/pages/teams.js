...
export async function renderTeams(el) {
  ...
  el.innerHTML = `<h1>Teams</h1>
    ${coached.length ? `<h3>Teams you coach</h3>${coached.map(t => `
      <div class="card tight">
        <div class="row between">
          <div><strong>${esc(t.name)}</strong><div class="muted small">${t.memberCount} rower${t.memberCount === 1 ? '' : 's'}</div></div>
          <a class="btn sm" href="#/team/${t.id}">Open</a>
        </div>
        <div class="row mt">
          <span class="badge blue" style="font-size:1rem;letter-spacing:2px">${esc(t.code)}</span>
          <button class="ghost sm" data-copy="${esc(t.code)}">Copy code</button>
          <button class="ghost sm" data-regen="${t.id}">Regenerate</button>
        </div>
    ...
    ${joined.length ? `<h3>Teams you row for</h3>${joined.map(t => `
      <div class="card tight list-item">
        <div style="flex:1"><strong>${esc(t.name)}</strong><div class="muted small">Coach: ${esc(t.coachName)}</div></div>
        <a class="btn sm secondary" href="#/team/${t.id}">Open</a>
        <button class="ghost sm" data-leave="${t.id}">Leave</button>
      </div>`).join('')}` : ''}

    ${state.user.accountType === 'rower' || true ? `
    <div class="card">
      <h3>Join a team</h3>
      <div class="row">
